...
- **20** — [JSON Schema export: annotation owns its projection](https://github.com/Ricci-curvature/reflecting-cpp26/blob/14830bc/stages/20_json_schema_export.cpp)
...
- [**One Declaration, Two Outputs**](https://riccilab.dev/blog/one-declaration-two-outputs) covers stage 20: Stage 18's `validate(v, ctx)` protocol gains a parallel `schema_emit<V>(sc)` member on every annotation — same `requires { … }` walker gate, same in-body `if constexpr` type dispatch, same consteval phase, different projection. `Range` → `"minimum":…,"maximum":…`, `MinLength` → `"minLength":…`, `NotNullopt` cross-cuts into the parent's `"required":[…]` list, and `Predicate<F>` exports as `"$comment":"predicate: …"` — honest about what JSON Schema can't enforce. Three probes pin the mechanics (nested concat at consteval, type dispatch covering `double` + `Range` → `"type":"number"`, and the two-bucket required-vs-properties pass). The public `av::json_schema<T>()` returns a `constexpr std::string` that `static_assert(==)` can pin byte-for-byte, so a validator and its schema can never drift. This is where the arc closes — one declaration, two consumers; twenty stages, ten posts, one header frozen.
